...
struct neV2
{
	...
	typedef const float* pcfloat;
        operator pfloat(){ return &v[0];}
	pcfloat  cfloat()const { return &v[0];}
	pfloat   data  (){ return &v[0];}

	NEINLINE void Set(const float X,const float Y){n.X=X;n.Y=Y;}        
	NEINLINE void SetZero(){Set(0,0);}

	neV2(){SetZero();};
	neV2(const float X,const float Y){n.X=X;n.Y=Y;}
	neV2(const float *A){if(!A)SetZero();else{n.X=A[0];n.Y=A[1];}}
	neV2(const neV2 &Src){(*this)=Src;}

	NEINLINE void operator = (const neV2 &Src){Set(Src.v[0],Src.v[1]);}
...
public:
	...
	neV3(){SetZero();};
	neV3(const float X,const float Y,const float Z){n.X=X;n.Y=Y;n.Z=Z;}
	neV3(const float *A){if(!A)SetZero();else{n.X=A[0];n.Y=A[1];n.Z=A[2];}}
	neV3(const neV3 &Src){(*this)=Src;}
    ...
    NEINLINE float  Normalize	(void);
	NEINLINE const neV3 NormalizeReturn 	(void)const;
    NEINLINE f32    Length      (void) const;
	NEINLINE f32    SqrLen      (void) const{return Dot(*this);}
	...
	NEINLINE neV3 friend operator + (const float* V1, const neV3& V2);
    NEINLINE neV3 friend operator - (const neV3& V1, const float *V2);
	NEINLINE neV3 friend operator - (const float* V1, const neV3& V2);
    NEINLINE neV3 friend operator + (const neV3& V1, const float *V2);
    ...
    NEINLINE neV3 friend operator * (const neV3& V,  f32 S);
	NEINLINE neV3 friend operator * (const neV3& V1, const neV3& V2);
//	NEINLINE float friend operator * (const neV3& V1, const neV3& V2);
	...
	NEINLINE void NormAngle();
	NEINLINE void Plus(float X,float Y,float Z);
	NEINLINE void normalize(float X,float Y,float Z);
	NEINLINE void SetTo(float *A)const;
	NEINLINE void SetFrom(float *A);
...
struct neV4
{
	...
	typedef float* pfloat;
	typedef const float* pcfloat;
    operator pfloat(){ return &X;}
	pcfloat  cfloat()const { return &X;}
    ...
    NEINLINE neV4 & SetZero ( void );
    NEINLINE neV4 & Set ( f32 x, f32 y, f32 z, f32 w );
...
struct neM4
{
	...
	NEINLINE void Set44(float row00, float row01, float row02, float row03
						, float row10, float row11, float row12, float row13
						, float row20, float row21, float row22, float row23
						, float row30, float row31, float row32, float row33);
	...
	NEINLINE friend neV4 operator *             ( const neM4& M1, const neV4& V  );
};
...
struct neQ
{
	...
	NEINLINE void slerp(const neQ &q0,const neQ &q1,const float t);
};
