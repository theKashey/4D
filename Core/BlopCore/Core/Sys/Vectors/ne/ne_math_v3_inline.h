...
NEINLINE float neV3::Normalize( void )
{
	f32 l = Length();
...
NEINLINE const neV3 neV3::NormalizeReturn	(void)const
{
	neV3 tem=*this;
	tem.Normalize();
	return tem;
...
NEINLINE neV3 operator + (const float* V1, const neV3& V2)
{
	neV3 tmp;

    return tmp.Set(V1[0] + V2.n.X, V1[1] + V2.n.Y, V1[2] + V2.n.Z );
}
NEINLINE neV3 operator - (const float* V1, const neV3& V2)
{
	neV3 tmp;
...
NEINLINE neV3 operator + (const neV3& V1, const float *V2)
{
	neV3 tmp;

    return tmp.Set( V1.n.X + V2[0], V1.n.Y + V2[1], V1.n.Z + V2[2] );
}
NEINLINE neV3 operator - (const neV3& V1, const float *V2)
{
	neV3 tmp;
...
//NEINLINE float operator * ( const neV3& V1, const neV3& V2 )
//{
//  return V1.Dot(V2);
//}
...
NEINLINE void neV3::NormAngle()
{
         while(n.X < 0   )n.X += 360;
		 while(n.X >= 360)n.X -= 360;
         while(n.Y < 0   )n.Y += 360;
		 while(n.Y >= 360)n.Y -= 360;
         while(n.Z < 0   )n.Z += 360;
		 while(n.Z >= 360)n.Z -= 360;
}

NEINLINE void neV3::Plus(float X,float Y,float Z)
{
	n.X+=X;
	n.Y+=Y;
	n.Z+=Z;
...
NEINLINE void neV3::normalize(float X,float Y,float Z)
{
    f32 l = Length();
	if (l == 0.0f)
		SetZero();
...
NEINLINE void neV3::SetTo(float *A)const
{
	for(int i=0;i<3;i++) A[i]=v[i];
}
NEINLINE void neV3::SetFrom(float *A)
{
