...
MemoryPool::MemoryPool(DWORD ByteCount,WORD _SlotSize,bool ChildBlock)
{
    if(!ChildBlock)
    {
     MemList::Add(this);
...
SlotPoint *MemoryPool::Alloc(DWORD NumBytes)
{
  ...
  //find slot
   ...
   if(!LinkedPool)
   {
	   ...
	   while((m>> dSlotSize)>0xFFFF) SlotSize=SlotSize<1;
	   LinkedPool=new MemoryPool(m,SlotSize,true);
   }
   if(LinkedPool)
...
bool MemoryPool::Optimize()
{
   ...
   if(NumOperations<2)return ret;
   //Print2Console(-1,"%x optimize...",this);
   NumOperations=0;
   FirstFreeBlock =0;
   LastBlockFree  =SlotsNum;
   int i=SlotsNum;
