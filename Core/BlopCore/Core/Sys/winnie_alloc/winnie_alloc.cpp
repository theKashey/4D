...
/*! \mainpage Winnie Alloc Library ver. 0.3
  ...
  multithreading application, you shold protect new/delete operations by critical sections by yorself.
...
const size_t max_os_block_num = 0x8000000000 / first_os_block_size; 
...
struct Header
{
	union 
	{
		...
		{
			unsigned char is_malloced; //is allocated by stdlib::malloc
...
//last element is alway NULL (for case when user call Winnie::Alloc
...
inline unsigned GetNextPowerOfTwo(unsigned k)
{
  --k;
...
/*template <bool throw_std_bad_alloc> struct OnOutOfMemory
{
  static void *F() { throw std::bad_alloc(); }
};

template <> struct OnOutOfMemory<false>
{
  static void *F() { return NULL; }
};
*/
...
bool FindNewBlockForList(unsigned list_num)
{
  ...
  { //we need to get new block from OS.
    ...
    //  (os_block_num+first_os_block_size_log > list_num)?
    //  os_block_num+first_os_block_size_log :
    //  list_num;
...
PVOID Alloc(size_t size)
{
  ...
  const unsigned list_num = GetNextPowerOfTwo(
	  static_cast<unsigned>(size)+sizeof(Header)); 
...
#endif
  {
	  if (!free_blocks_lists[list_num])
	  {
		  ...
		  {
			  return NULL;//OnOutOfMemory<Config::throw_std_bad_alloc>::F();
		  }
	  }
